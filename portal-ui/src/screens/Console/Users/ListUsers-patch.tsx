...
const AddUser = withSuspense(React.lazy(() => import("./AddUser-patch")));
...
async function fetchUserList():Promise<UsersList> {
  const { groups } = await  api.invoke("GET", `/api/v1/groups`);
  console.log(groups);

  let users = [];

  for (const groupName of groups) {
    const {members} = await api.invoke("GET", `/api/v1/group?name=${encodeURI(groupName)}`);
    console.log(members);
    for(const member of members){
      const user:User = await api.invoke("GET", `/api/v1/user?name=${encodeURI(member)}`);
      users.push(user);
    }
  }
  return {
...
const ListUsers = ({ classes, setErrorSnackMessage, history }: IUsersProps) => {
  ...
  useEffect(() => {
    if (loading) {
      if (displayListUsers) {
        fetchUserList().then((res: UsersList) => {
